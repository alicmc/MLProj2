...
def main():
    ...
    SMALL_MAMMAL_PRIORITY = 2.5  # Extra priority for Small_mammal
    ...
    print("="*80)
    print("TRAINING WITHOUT FROG CLASS")
    print("="*80)
    ...
    # -------------------------
    # 3. Dataset - EXCLUDING FROG CLASS
    # -------------------------
    # Original class map (keep indices consistent)
    class_map = {
        'Big_mammal':0, 'Bird':1, 'Frog':2,
        'Lizard':3, 'Scorpion':4, 'Small_mammal':5, 'Spider':6
    }
    
    # Classes we're actually training on (6 classes, no Frog)
    active_classes = ['Big_mammal', 'Bird', 'Lizard', 'Scorpion', 'Small_mammal', 'Spider']
    FROG_ID = class_map['Frog']

    print(f"\n🚫 EXCLUDING FROG CLASS from training")
    print(f"Training on 6 classes: {', '.join(active_classes)}")
    ...
    # Filter out Frog samples
    non_frog_indices = []
    for idx in range(len(temp_dataset)):
        _, label = temp_dataset[idx]
        if label != FROG_ID:  # Exclude Frogs
            non_frog_indices.append(idx)
    
    print(f"Filtered dataset: {len(non_frog_indices)} samples (removed {len(temp_dataset) - len(non_frog_indices)} Frogs)")
    
    # Split indices
    val_size = int(0.2 * len(non_frog_indices))
    train_size = len(non_frog_indices) - val_size
    ...
    # -------------------------
    print("\nCalculating class distribution for balanced sampling (NO FROG)...")
    ...
    print("\nTraining set class distribution (NO FROG):")
    class_names_reverse = {v: k for k, v in class_map.items()}
    for class_id in sorted(class_counts.keys()):
        if class_id != FROG_ID:  # Skip Frog in display
    ...
    print("Note: Model has 7 output classes, but Frog class will not be trained")
    ...
    for class_id in range(num_classes):
        if class_id == FROG_ID:
            adaptive_weights[class_id] = 0.0  # Zero weight for Frog (not trained)
        elif class_id in class_counts:
            # Inverse frequency: rare classes get higher weight
            adaptive_weights[class_id] = max_count / class_counts[class_id]
        else:
            adaptive_weights[class_id] = 0.0
    
    # Apply extra multiplier for Small_mammal
    ...
    for epoch in range(EPOCHS):
        ...
        # Track per-class accuracy (excluding Frog)
        class_correct = {i: 0 for i in range(num_classes) if i != FROG_ID}
        class_total = {i: 0 for i in range(num_classes) if i != FROG_ID}
        ...
        print(f"\nPer-Class Training Accuracy (NO FROG):")
        for class_id in range(num_classes):
            if class_id != FROG_ID and class_total.get(class_id, 0) > 0:
                class_acc = 100 * class_correct[class_id] / class_total[class_id]
                class_name = class_names_reverse[class_id]
                marker = "🎯" if class_id == class_map['Small_mammal'] else "  "
        ...
        # Track per-class validation accuracy (excluding Frog)
        val_class_correct = {i: 0 for i in range(num_classes) if i != FROG_ID}
        val_class_total = {i: 0 for i in range(num_classes) if i != FROG_ID}
        ...
        print(f"\nPer-Class Validation Accuracy (NO FROG):")
        small_mammal_acc = 0
        for class_id in range(num_classes):
            if class_id != FROG_ID and val_class_total.get(class_id, 0) > 0:
        ...
        print(f"\n🎯 Priority Class Performance:")
        print(f"   Small_mammal: {small_mammal_acc:.1f}%")

        # ---- Early Stopping Check ----
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_val_loss = val_loss
            torch.save(model.state_dict(), "best_swin_model_no_frog.pth")
            print(f"\n✓ Best model saved! (Val Acc: {val_acc:.2f}%, Small_mammal Acc: {small_mammal_acc:.1f}%)")
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
    ...
    print("\nTraining complete. Best model saved as best_swin_model_no_frog.pth")
    ...
    ax1.set_title('Training and Validation Loss (NO FROG)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Training and Validation Accuracy (NO FROG)', fontsize=14, fontweight='bold')
    ax2.legend(fontsize=10)
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('training_curves_no_frog.png', dpi=300, bbox_inches='tight')
    print("Training curves saved as 'training_curves_no_frog.png')")
    plt.show()
    
    print(f"\n{'='*80}")
    print("FINAL RESULTS (TRAINED WITHOUT FROG CLASS)")
    print(f"{'='*80}")
    print(f"Best Validation Accuracy: {best_val_acc:.2f}%")
    ...
    print(f"{'='*80}")
    print("\n🚫 Note: This model was trained EXCLUDING all Frog samples")
    print("   Model saved as: best_swin_model_no_frog.pth")
    print("   Training curves: training_curves_no_frog.png")
    print("   Test with: test_swin_no_frog.py")
